...
export function openStore(directory) {
  ...
  // A crashed process may have already incurred an API charge. Keep its reservation.
...
export function validateLimits(input, partial = false) {
  ...
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('Некорректные лимиты.');
  const out = {};
  for (const [key, [min, max]] of Object.entries(ranges)) {
    if (partial && input[key] === undefined) continue;
    const n = input[key];
    if (typeof n !== 'number' || !Number.isFinite(n) || n < min || n > max || (key !== 'dailyUSD' && !Number.isInteger(n))) throw new Error(`Недопустимый лимит: ${key} (${min}–${max}).`);
    out[key] = n;
  }
  return out;
}
